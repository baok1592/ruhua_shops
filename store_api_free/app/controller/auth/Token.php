...
//        $request = cache($key, $value, $this->tokenExpire);//第三参数是时效期
    ...
    public function get_sin_token($pulg='')
    {
        $get=(new BaseCommon())->curl_get(WEB_URL.'/Sass/tf_file?pulg='.$pulg);
        $get1=json_decode($get,true);
        $post=$get1['data'];
        $token=(new Token())->getCloudToken();
        if ($token==null&&$post['token']!=null){

            $res=(new Token())->SaveCloudToken($post['token']);
        }
        if($post['code']==500)
        {
            Cache::clear();
