...
class Favorites extends BaseModel
{
    ...
    public static function addFavorites($data){
        ...
        if($data['type']=='goods'){
            $imgs=GoodsImgs::where('goods_id',$data['fav_id'])->find();
            if($imgs)
                $data['img']=$imgs['thumb_img'];
        }else{
            $shop=Shop::where('shop_id',$data['fav_id'])->find();
            if ($shop){
                $img=Image::where('id',$shop['image_id'])->find();
    ...
    public static function getFavoriteByUid($uid)
    {
        $data=self::with(['product'=>function($query){
            $query->field('goods_id,goods_name,price');
