...
class Goods extends BaseModel
{
    ...
    {
        return $this->belongsTo('GoodsImgs', 'goods_id', 'goods_id');
    }
    //关联视频
    ...
    public function fx()
    {
        return $this->belongsTo('FxGoods','goods_id','goods_id')->field('goods_id');
    }
    ...
    {
        $goods=self::with('ordergoods')->select();
        foreach ($goods as $k=>$v){
            $v['num']=self::CountType($v['ordergoods'],'num');
            $v['total_price']=self::CountType($v['ordergoods'],'total_price');
            unset($goods[$k]);
            unset($v['ordergoods']);
            $goods[$k]=$v;
        }
        $goods=self::goods_sort($goods,'num');
        return $goods;
    }

    public static function CountType($data,$type){
        $sum=0;
        foreach ($data as $k=>$v){
            $sum+=$v[$type];
        }
     ...
     * 总订单数排名
     */
    private static function goods_sort($shop,$type)
    {
