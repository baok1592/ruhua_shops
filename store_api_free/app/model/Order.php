...
class Order extends BaseModel
{
    ...
    public static function deleteOrder($id, $uid)
    {
        ...
        if ($order['user_id'] == $uid && $order['payment_state'] == 0) {
            ...
            {
               UserCouponModel::update(['status'=>0],['id'=>$order['coupon_id']]);
            }
    ...
    public static function receive($id, $uid)
    {
        ...
        if($order['invite_code']&&$order['invitecode']['id']){
            ...
            $data=[
                ...
                'agent_id'=>$order['invitecode']['id']?$order['invitecode']['id']:0,
                'money'=>$money,
            ...
            $fx_data=[
                'shop_id'=>$order['shop_id'],
                'price'=>($order['order_money']-$data['money']),
            ];
            $shop=ShopRecordModel::where('shop_id',$order['shop_id'])->find();
            if(!$shop){
                ShopRecordModel::create($fx_data);
            }else{
                ShopRecordModel::update(['price'=>($fx_data['price']+$shop['price'])],['shop_id'=>$fx_data['shop_id']]);
            }
        }
...
//            return $res ? 1 : 0;
    ...
    public static function shop_delete($order_id)
    {
        $order=self::where('order_id',$order_id)->find();
        if($order){
            $res=$order->delete();   //这里是软删
            $sql=self::getLastSql();
            Log::error($sql);
            return $res;
        }
