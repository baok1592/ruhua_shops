...
class CommonServices
{
    ...
    public static function uploadImg($use, $back, $type = '', $cid = '')
    {
        ...
        $name = uniqid() . '.png';
        $file->thumb(1500, 1500, 1)->save('./uploads/' . $use . '/' . $name);
    ...
    public static function uploadVideo()
    {
        ...
        validate(['file' => 'fileSize:15360000'])
            ->check(['file' => $file]);
    ...
    private static function img_save($use, $name, $cid = '')
    {
        ...
        $data['sid']=TokenService::getShopId1();
        $is_oss=SysConfigModel::where('key','upload_oss')->value('value');
        if($is_oss==1){
            $oss=new Oss();
            $data['url']=$oss->uploadFile(ROOT.$data['url']);
            $path=ROOT.$data['url'];
            if(file_exists($path))
                unlink($path);//删除文件
        }
    ...
    public static function getShopCodeUrl($path)
    {
        $url=(new QrcodeServer())->getCodeUrl($path);
        return $url;
