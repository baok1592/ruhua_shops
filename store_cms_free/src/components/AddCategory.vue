...
export default {
  ...
  data() {
    return {
      rules: {
        category_name: [
          { required: true, message: "请输入分类名称", trigger: "blur" } //blur失去焦点
        ],
        short_name: [
          { required: true, message: "请输入分类简称", trigger: "blur" } //change值发送改变
        ],
        pid: [{ required: true, message: "请选择分类等级", trigger: "change" }]
      },
      length: 1,
      drawer: false,
      getimg: this.$getimg,
