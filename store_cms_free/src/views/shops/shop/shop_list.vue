					...
					<div class="zhang">
						...
						<transition appear appear-active-class="animated fadeInLeft">
							<el-table :data="list" border style="width: 100%">
								...
								<el-table-column label="抽佣比" width="180">
									<template slot-scope="scope">
										{{scope.row.chouyong>0?scope.row.chouyong+'%':''}}
									</template>
								</el-table-column>
								...
								<el-table-column label="操作">
									<template slot-scope="scope">
										...
										<el-button v-else  style="margin-left: 10px" type="info" size="small"
										 slot="reference" @click="open_store(scope.row.shop_id)">恢复</el-button>
		...
		methods: {
			...
			open_store(id){
				this.$confirm("是否恢复?", "提示", {
					...
					StoreModel.open_store({id:id}).then(res=>{
						this.$message({
							message: '操作成功',
							type: 'success'
						});
