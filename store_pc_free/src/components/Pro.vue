								...
								<el-form-item label="商品分类">
									<el-select v-model="forms.category_id" placeholder="请选择商品分类">
										<template v-for="item in cate_list">
											<el-option  :value="item.category_id" :label="item.category_name" :key="item.category_id"></el-option>
										</template>
									</el-select>
								</el-form-item>
							</el-col>
		...
		methods: {
			...
			get_img(e) {
				...
				for (let k in e) {
					...
					// this.forms.banner_imgs.push(v.url)
				}
			...
			del_img_detail(index) {
				...
				this.forms.detail_imgs.splice(index, 1)
			},
			get_delivery() {
			...
			async onSubmit() {
				...
				if (that.is_caiji == 1) {
					...
					this.forms.banner_imgs = this.img_list
				}
				if(!this.check_data()){
					return
				}
				...
				this.forms.detail_imgs = this.img_list_detail
				this.forms.video_id = this.video_id
				this.http.post_show("pro/pro_add", that.forms).then(res => {
			...
			editProduct(id) {
				...
				this.http.get('pro/pro_detail?id=' + id).then(res => {
console.log(res.data)
					...
					this.forms.content = res.data.goods_imgs.content
					this.forms.tag_ids = res.data.tag_ids
					
					this.forms.banner_imgs = res.data.goods_imgs.banner_imgs
					this.img_list = res.data.goods_imgs.banner_imgs
					this.forms.detail_imgs = res.data.goods_imgs.detail_imgs
					this.img_list_detail = res.data.goods_imgs.detail_imgs
					if(!this.forms.detail_imgs){
						this.forms.detail_imgs = []
			...
			onEdit() {
				var that = this;
				if(this.show_sku == 1){
					that.forms.sku = this.sku_obj
				}
				
				console.log(this.forms)
				this.forms.detail_imgs = this.img_list_detail
				this.http.post_show("pro/pro_up", that.forms).then(() => {
					this.$message({
						showClose: true,
						message: "修改成功",
						type: "success"
