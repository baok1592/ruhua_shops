	...
	export default {
			...
			tableList: function() {
				...
				for (let i = 0; i < rows; i++) {
					...
					for (let j = 0; j < srcData.length; j++) {
						...
						let key = "s"+(j*1+1);
						let name = "s"+(j*1+1) +"_name";
						let rowspan = srcData[j]["rowspan"];
						let len = srcData[j]["specLen"];
						if (!len) {
							continue;
						}
						for (var p = 0; p < srcData[j].spec.length; p++) {
							srcData[j].spec[p]["id"] = "10"+(p*1+1)
						}
						let spec = srcData[j]["spec"];
						let index = parseInt(i / rowspan) % len;
						listItem[key] = spec[index]["id"];
						listItem[name] = spec[index]["cName"];
					}
					listItem["price"] = "";
					listItem["stock_num"] = "";
		...
		methods: { 
			...
			add_canshu(index) {
				...
				this.sku_obj.tree[index].v.push({
					id:"10"+this.k,
					name:''
				})
				console.log(this.sku_obj)
