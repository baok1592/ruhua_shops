					...
					<div class="zhang">
						...
						<div v-if="!detail_show" style="padding: 15px;background-color: #fff">
							<el-table :data="list" border style="width: 100%" @filter-change="xxx">
								...
								<el-table-column label="支付状态" width="90" :filters="[{ text: '已支付', value: 1 }, { text: '未支付', value: 0 }]"
								...
								<el-table-column label="订单状态" width="90" :filters="[{ text: '已退款', value: -2 }, { text: '退款中', value: -1 },{ text: '未完成', value: 0 },{ text: '已完成', value: 1 },{ text: '已评价', value: 2 },]"
								...
								<el-table-column label="运输状态" width="90" :filters="[{ text: '待发货', value: 0 }, { text: '已发货', value: 1 },{ text: '已收货', value: 2 }]"
								...
								<el-table-column prop="operation" label="操作" >
									<template slot-scope="scope">
										...
										<el-button style="margin-left: 10px" type="success" size="small" @click="print_order(scope.row.order_id)">打印</el-button>
									</template>
								</el-table-column>
							</el-table>
...
import {
	...
	export default{
		...
		methods: {
			...
			get_all_order() {
				const that = this
				this.http.post('order/order_list').then(res => {
					that.all = res.data
					that.list = res.data;
