		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+'px'}">
			...
			<view v-if="!inputVal">
				<view class="current-city">
					...
					<view class="city-name" @click="dinwei(localCity.district)">
				...
				<view class="hot-city">
					...
					<view class="city-names">
						<view class="city-name-item" v-for="(item,index) in hotCity" :key="index" hover-class="tap-city" :hover-stay-time="150"
						 @tap="selectCity" :data-name="item.region_name">
							{{item.region_name}}
						</view>
					</view>
				</view>
...
<script>
	const cityData = require('@/util/city.js')
	...
	export default {
		...
		data() {
			return {
				...
				hotCity: [
					'兴义市'
					// '北京', '上海', '广州', '深圳', '杭州', '长沙', '武汉', '厦门', '西安', '昆明', '成都', '重庆'
				], // 热门城市
		...
		onLoad: function(options) {
			const that = this; 
			this.type=options.type=='shop'?'shop':'';
			console.log('t:',options,this.type)
			...
			setTimeout(() => {
				...
				uni.getLocation({
					type: 'wgs84',
					success: function(res) {
						that.$api.http.get('shops/city/position?coords=', {
							coords: res.latitude + ',' + res.longitude //纬度+经度
						}).then(res => {
							console.log(res)
							that.localCity = res.data
						})
					}
				});
				this.get_hotcity()
		...
		methods: {
			//获取热门城市
			get_hotcity(){
				this.$api.http.get('shops/city/web_city').then(res=>{
					// console.log(res)
			...
			dinwei(name){
				...
				// this.$eventHub.$emit('emit', name)
				uni.navigateBack({
					// delta: 1
				})
				// this.$api.http.post('up_region_name', {
				// 	region_name: name
				// }).then(res => {
			...
			selectCity(e) {
				...
				if(this.type!='shop'){
					...
					uni.setStorageSync('reg_city',cityName)
					// const region=uni.getStorageSync('region')
					// for (let k in region) {
					// 	const v=region[k] 
					// 	if(v.name==cityName){
					// 		lock=false							
					// 		uni.setStorageSync('reg_city',{id:v.id,name:cityName})
					// 	}
					// }
					// if(lock){
					// 	this.$api.msg('该地区未开通')
