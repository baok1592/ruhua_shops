					...
					<view v-else>
						<view v-for="(item,index) in order_list" :key="index" class="order-item" v-if="tabCurrentIndex==0 || (item.payment_state==now_pay && item.shipment_state==now_drive && item.state==now_state) ">
							<view class="i-top b-b">
									...
									<text class=" unfold cuIcon-shop" ></text>
									<text class="time" @click="jump_shops(item.shop_id)">
										<block v-if="item.shops && item.shops.shop_name" >
											{{item.shops.shop_name}}
											<text class=" unfold cuIcon-right" ></text>
										</block>
									</text>
							...
							<scroll-view v-if="item.order_goods.length > 1" class="goods-box" scroll-x>
								<view v-for="(goodsItem, goodsIndex) in item.order_goods" :key="goodsIndex" class="goods-item" @click="jumo_tomyorder(item.order_id)">
									<image class="goods-img" :src="getimg+goodsItem.goods_imgs.thumb_img" mode="aspectFill"></image>
								</view>
							</scroll-view>
							<view v-if="item.order_goods.length === 1" class="goods-box-single" v-for="(goodsItem, goodsIndex) in item.order_goods"
							 :key="goodsIndex">
								<image class="goods-img" :src="getimg+goodsItem.goods_imgs.thumb_img" mode="aspectFill" @click="jumo_tomyorder(item.order_id)"></image>
...
<style lang="scss">
	...
	.order-item {
		...
		.i-top {
			...
			color: $font-color-dark;
			position: relative;
			.cuIcon-shop{font-size: 20px;padding-right: 5px;}
			.time {
				flex: 1;font-weight: 600;font-size: 15px;
				.cuIcon-right{color: #B7B7B7;}
			}
